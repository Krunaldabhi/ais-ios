...
class otpverfiyVC: UIViewController,PostViewProtocol {
    ...
    @IBAction func verfiyBtn(_ sender: UIButton) {
        ...
        {
            self.pinstr = "\(textField1.text!)\(textField2.text!)\(textField3.text!)\(textField4.text!)\(textField5.text!)\(textField6.text!)"
            
            let defaults = UserDefaults.standard
            let credetial: PhoneAuthCredential = PhoneAuthProvider.provider().credential(withVerificationID: defaults.string(forKey: "authID")!, verificationCode: pinstr)
            
            Auth.auth().signIn(with: credetial) { (user, error) in
                if error != nil
                {
