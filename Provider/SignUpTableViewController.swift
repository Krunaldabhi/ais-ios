...
class SignUpTableViewController: UITableViewController, UIGestureRecognizerDelegate {
    ...
    @IBOutlet var textFieldOwnerType: HoshiTextField!
    ...
    override func viewWillAppear(_ animated: Bool) {
      ...
      //  self.presenter?.get(api: .LimoOwner, parameters: nil)
        self.presenter?.post(api: .LimoOwner, data: nil)
    }
...
extension SignUpTableViewController {
    ...
    private func localization(){
        ...
        self.textFieldOwnerType.placeholder = Constants.string.ownerType.localize()
...
extension SignUpTableViewController {
    ...
    @objc private func nextIconTapped(sender: UITapGestureRecognizer){
        ...
        guard let ownerType = self.textFieldOwnerType.text, !ownerType.isEmpty else {
            vibrate(sound: .cancelled)
            textFieldOwnerType.shake()
            UIApplication.shared.keyWindow?.make(toast: ErrorMessage.list.enterOwenrType.localize())
            return
        }
        ...
        let number =  "\(self.countryText.text!)\(phoneNumber)"
        ...
        let action = UIAlertAction(title: "Yes", style: .default) { (UIAlertAction) in
            PhoneAuthProvider.provider().verifyPhoneNumber(number, uiDelegate:nil) { (verificationID, error) in
                if  error != nil {
                    ...
                    self.userSignUpInfo = MakeJson.SingUp(firstName: firstName, lastName: lastName, email: email, mobile: Int(phoneNumber) ?? 0, password: password, ConfirmPassword: password, device_Id: deviceId, device_type: DeviceType.ios.rawValue, device_token: deviceTokenString, referral_code: self.isReferalEnable == 0 ? "" : self.textFieldReferCode.text!, service_type: String(self.selectedServiceID), service_number: vehicleNumber, service_model: self.textFieldModel.text!, country_code: self.countryText.text!,fleet_id:String(self.selectedownerID))
                    
                    let vc = self.storyboard?.instantiateViewController(withIdentifier: "otpverfiyVC") as! otpverfiyVC
                    vc.userSignUpInfo = self.userSignUpInfo
...
//        let akPhone = PhoneNumber(countryCode: "in", phoneNumber: phoneNumber)
////        let akPhone = PhoneNumber(countryCode: "in", phoneNumber: phoneNumber)
...
extension SignUpTableViewController: UITextFieldDelegate {
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        ...
        if textField == textFieldOwnerType {
            ownerTypeView.anchorView = textField
            var dataSource:[String]=[]
            for owner in self.ownerTypeArr! {
                dataSource.append(owner.company!)
            }
            ownerTypeView.dataSource = dataSource
            ownerTypeView.selectionAction = { [weak self] (index, item) in
                self?.textFieldOwnerType.text = item
                self?.selectedownerID = (self?.ownerTypeArr![index].id)!
            }
            DropDown.setupDefaultAppearance()
            ownerTypeView.cellNib = UINib(nibName: "DropDownCell", bundle: Bundle(for: DropDownCell.self))
            ownerTypeView.show()
            return false
        }
...
//        self.isReferalEnable = Int((data.referral?.referral)!)!
    ...
    func getowner(api: Base, data: [ownerTypes]) {
        self.ownerTypeArr = data
