            ...
            if self.datasource.count > indexPath.section {
                ...
                tableCell.onclickAdd = { [weak self] documentId, completion in
                    ...
                    self.showImage(with: { (image) in
                        ...
                        let compressData = image!.jpegData(compressionQuality: 0.1) //max value is 1.0 and minimum is 0.0
                        let compressedImage = UIImage(data: compressData!)
                        var imageObject = compressedImage
                        ...
                        if imageObject != nil, var imageData = imageObject!.pngData() {
                           ...
                           // while size>2048{
                                ...
                                if let imageResized = imageObject?.resize(with: 0.1),let imageObjectData = imageResized.pngData() {
